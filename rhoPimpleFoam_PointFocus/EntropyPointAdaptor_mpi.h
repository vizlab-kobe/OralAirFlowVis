/*****************************************************************************/
/**
 *  @file   EntropyPointAdaptor_mpi.h
...
class EntropyPointAdaptor : public InSituVis::mpi::Adaptor, public local::EntropyTimestepController
{
public:
...
public:
    EntropyPointAdaptor( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root ) {}
    virtual ~EntropyPointAdaptor() = default;
...
protected:
    ...
    coord regionEntropy(const BaseClass::FrameBuffer& frame_buffer,int M,int N);
...
#include "EntropyPointAdaptor_mpi.hpp"
