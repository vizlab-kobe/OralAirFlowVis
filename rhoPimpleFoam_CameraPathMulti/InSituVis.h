...
// Adaptor definition
//----------------------------------------------------------------------------
namespace { using Adaptor = InSituVis::mpi::CameraPathControlledAdaptorMulti; }
...
const auto FocusPointCandidates = 1;
...
public:
    InSituVis( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root )
    {
        ...
        this->setFocusPointCandidateNum( Params::FocusPointCandidates );
        this->setViewPointCandidateNum( Params::ViewPointCandidates );
        this->setROIMethod( Params::ROImethod );
        this->setViewDim( Params::ViewDim );
    ...
    void execRendering()
    {
        ...
        if ( bbox ) { bbox->setVisible( visible && Params::VisibleBoundingBox ); }
            ...
            if ( BaseClass::isEntStep() && !BaseClass::isErpStep() )
            {
                ...
                for( size_t i = 0; i < BaseClass::focusPointCandidateNum(); i++ ){
