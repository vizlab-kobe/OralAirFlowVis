...
#include <InSituVis/Lib/CameraFocusControlledAdaptorMulti_mpi.h>
...
// Adaptor definition
//----------------------------------------------------------------------------
namespace { using Adaptor = InSituVis::mpi::CameraFocusControlledAdaptorMulti; }
      ...
      if (Params::EstimateIncludingBox == false)
        ...
        if ( bbox ) { bbox->setVisible( visible && Params::VisibleBoundingBox ); }
            ...
            if ( BaseClass::isEntStep() && !BaseClass::isErpStep() )
            {
                ...
                for( size_t i = 0; i < BaseClass::candidateNum(); i++ ){
